...
const totalSum = (input) => {
  // Solution code here...
  return input.reduce( (sumArr, nextArr) => sumArr + nextArr.reduce( (sumEle, nextEle) => sumEle + nextEle, 0), 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.map( arr => arr.filter( ele => typeof ele === 'number' && ele % 5 === 0).map(num => Math.pow(2, num)));
};
...
let findShortest = (data) => {
  // Solution code here...
  return data.reduce( (prev, curr) => parseInt(curr.height) < parseInt(prev.height) ? curr : prev).name;
};
