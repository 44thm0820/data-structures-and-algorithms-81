...
const getStatName = (arr, minBaseStat) => {
  // Solution code here...
  return arr.filter(statObj => statObj.baseStat > minBaseStat).map(statObj => statObj.stat.name);
};
...
const evenOddNumericValues = (arr) => {
  // Solution code here...
  return arr.filter(ele => Number.isInteger(ele)).map(int => int % 2 === 0 ? 'even' : 'odd');
};
