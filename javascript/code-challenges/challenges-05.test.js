...
const addPurchases = (arr) => {
  // Solution code here...
  return arr.reduce( (prev, curr) => {
    return prev + curr.purchasePrice;
  }, 0);
};
...
const calculateAverage = (arr) => {
  ...
  return obj.count > 0 ? obj.sum / obj.count : 0;
};
...
const extractStat = (statName, arr) => {
  // Solution code here...
  return arr.reduce( (prev, curr) => {
    if (prev === null && curr.stat.name === statName) {
      prev = curr;
    }
    return prev;
  }, null);
...
const extractChildren = (arr) => {
  // Solution code here...
  let charactersWithA = arr.filter(obj => obj.name.toLowerCase().includes('a'));
  return charactersWithA.reduce( (prev, curr) => {
