...
const raisedToTheThird = (arr) => {
  // Solution code here...
  // arr.map( num => Math.pow(num, 3));
  let arrResult = [];
  arr.forEach( num => arrResult.push(Math.pow(num, 3)) );
  return arrResult;
};
...
const forLoopTwoToThe = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    arrResult.push(Math.pow(2, arr[i]));
  }
  return arrResult;
};
...
const forEachTwoToThe = (arr) => {
  // Solution code here...
  let arrResult = [];
  arr.forEach (num => arrResult.push(Math.pow(2, num)));
  return arrResult;
};
...
const extractStats = (arr) => {
  let makeStatObjects = statObj => {
    let obj = {};
    obj.name =statObj.stat.name;
    obj.total = statObj.effort + statObj.baseStat;
    return obj;
  };
  return arr.map(makeStatObjects);
};
